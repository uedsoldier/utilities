...
static const struct ip_eth_addr broadcast_ethaddr =
...
ip_arp_timer(void)
{
  ...
  for(i = 0; i < IP_ARPTAB_SIZE; ++i) {
    ...
    if((tabptr->ipaddr[0] | tabptr->ipaddr[1]) != 0 &&
       arptime - tabptr->time >= IP_ARP_MAXAGE) {
...
ip_arp_ipin(void)
{
  ...
  if((IPBUF->srcipaddr[0] & ip_netmask[0]) !=
     (ip_hostaddr[0] & ip_netmask[0])) {
    return;
  }
  if((IPBUF->srcipaddr[1] & ip_netmask[1]) !=
     (ip_hostaddr[1] & ip_netmask[1])) {
    return;
  }
